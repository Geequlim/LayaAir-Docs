...
4. 脚本的 `onAwake()` 执行一次
5. 脚本的 `onEnable()` 执行
6. 第一轮 `onUpdate()` 之前，脚本的 `onStart()` 执行一次
7. 后续每帧才进入 `onUpdate()` / `onLateUpdate()`
...
`@regClass()` 最容易被误解成“写脚本就必须加，不加完全不能跑”。
...
1. 代码自己知道要创建哪个类
2. 引擎直接 `new` 这个类并挂到节点上

这条路径的关键不是“按名字查类”。

### 路径二：场景、prefab、runtime 数据要求运行时按名字恢复类

另一条常见路径是：

1. 场景或 prefab 数据里记录了某个脚本类型
2. 节点声明了某个 `runtime` 类
3. 运行时在反序列化时，需要把字符串或类型标记还原成真正的类

这时类注册就是硬边界。

如果类没有注册成功，或者发布结果里这个类根本没有被带进运行时，那么常见后果就是：

1. 资源文件加载成功
2. 节点树也可能创建出来
3. 但脚本绑定或 `runtime` 替换失败

所以 `@regClass()` 解决的不是“让普通代码能继承 `Laya.Script`”，而是“让运行时能够在反序列化阶段认出这个类”。

## 为什么类注册和“是否被构建保留”还要分开看
...
1. 类已经注册到可查询的类表里
2. 发布后的代码里，这个类本身真的还存在

如果一个类只出现在场景数据里，但最终产物里没有把这段脚本代码带进来，那么运行时同样无法正确恢复。

所以最稳妥的理解是：

1. 直接代码引用，主要依赖类本身可用
2. 序列化绑定和 `runtime` 恢复，既依赖类可用，也依赖类注册
...
对 `LayaAir 3.4` 来说，节点和脚本一起工作的关键，不是多记几个生命周期名字，而是先建立这条主线：
